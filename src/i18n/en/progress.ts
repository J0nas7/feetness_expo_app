export default {
  deleteWorkout: { action: 'Delete workout', title: 'Delete this workout?' },
  monthlyPlan: 'Monthly plan',
  editMonthlyPlan: 'Edit monthly plan for {{period}}',
  workoutGoal: '{{amount}} {{unit}} goal',
  hours: 'hours',
  minutes: 'min',
  currentMonth: 'Current month',
  yearOverview: '{{year}} overview',
  monthlyPace: '{{remaining}} {{unit}} remaining · {{days}} days left · minimum {{daily}} {{unit}} per day',
  viewWorkoutList: 'View workout list',
  workoutListTitle: 'Workouts',
  emptyWorkoutList: 'No workouts in this period.',
  calendar: {
    title: 'Workout Calendar',
    subtitle: 'Browse workouts by day, week, month, or year',
    open: 'Open workout calendar',
    today: 'Today',
    loading: 'Loading calendar…',
    noWorkouts: 'No workouts',
    weekTitle: 'Week {{week}}, {{year}}',
    views: { day: 'Day', week: 'Week', month: 'Month', year: 'Year' },
  },
  toolbar: { selected: '{{count}} selected', selectAll: 'Select all', deselectAll: 'Deselect all', edit: 'Edit', done: 'Done' },
  bulkDelete: { title: 'Delete {{count}} selected workout?', titlePlural: 'Delete {{count}} selected workouts?', warning: 'This action cannot be undone.' },
  accessibility: { selectWorkout: 'Select workout', deleteSelected: 'Delete {{count}} selected workouts', editSelected: 'Edit {{count}} selected workouts' },
  period: {
    selector: { week: 'WEEK', month: 'MONTH' },
    week: 'Week {{week}} ({{date}})',
    weekShort: 'W{{week}}',
  },
  metrics: {
    workouts: 'Number of workouts',
    distance: 'Total distance (km)',
    duration: 'Total duration (min)',
    goals: 'Goals completed',
    pace: 'Average pace (min/km)',
  },
  summary: {
    completedGoal: '{{count}} goal',
    completedGoals: '{{count}} goals',
    goalsShort: '{{count}} goals',
  },
} as const;
